...
La suite du document est structurée comme suitNous allons pour ce faire déjà présenter dans la section~\ref{sec:generality} des généralités sur les graphes et les descripteurs extraits des graphes. Ensuite, nous présenterons dans la section~\ref{sec:mlna} la modélisation en graphes multicouches et les descripteurs extraites de ces graphes. Enfin, nous présenterons quelques expérimentations réalisées et les résultats obtenus à la section~\ref{sec:exp} et nous achèverons par une conclusion sur le travail fournis et quelques perspectives dans la section~\ref{sec:con}.
...
Après la construction d’un graphe, il est possible d’extraire des variétés de descripteurs liées soit aux nœuds et donc aux entités, soit à la relation entre les nœuds, et même à la topologie du graphe.
\begin{itemize}
...
La lecture de l’article « Multilayer network analysis for improved credit risk prediction » de Mar{\'i}a {\'O}skarsd{\'o}ttir et Cristi{\'a}n Bravo\cite{oskarsdottir2021multilayer}, a été le point d’entrée pour l’usage des descripteurs issus de la modélisation en graphe, pour enrichir les données d’apprentissage des modèles classiques de prédiction du risque de crédit.
...
Dans le cas de l’article qui s’attarde sur les prêts agricoles, les deux dimensions choisies sont : la localisation géographique et les produits vendus par les agriculteurs.
...
%\subsubsection{Attributs du MLN1 - Fonction \& Civilité}
	...
	\begin{itemize}
		\item \textbf{MLN\_Att1\_degré :} le nombre d'emprunteur avec la même valeur d'Att
		\item \textbf{MLN\_Att2\_degré :} le nombre d'emprunteur avec la même valeur d'Att
		\item \textbf{MLN\_Att1\_et\_Att2\_degré :} le nombre d'emprunteur avec la même valeur Att1 et Att2
...
Nous avons considéré cinq modèles classiques de l’apprentissage automatique pour la prédiction du risque de crédit : \ac{DT}, \ac{RF}, \ac{LR}, \ac{XGB} et \ac{SVM}.
...
autres attributs extraits du graphe multicouches \ac{MLN}
	...
	\item \textbf{Classic + MLN - Att} : on écarte les attributs relatifs aux dimensions du graphe multicouche MLN, et on intègre les attributs extraits du graphe multicouche \ac{MLN}
\end{itemize}
...
\begin{figure}[H]
  ...
  \stepcounter{tableau}
  \captionsetup{labelformat=tableau}
  \caption{Récapitulatif des résultats avec les différents graphes multicouches}
  \label{fig:afbR}
\end{figure}


Lorsqu’on s’attarde sur le classement des modèles de prédiction, le modèle de prédiction associé à la meilleure performance est la Forêt Aléatoire (Random Forest), suivi d’Arbre de décision et de \ac{XGB}. La Régression Logistique et SVM ferme ce classement.
...
\textbf{Evaluation de la contribution des attributs pour les modèles de prédiction}
...
En observant les graphiques des résultats des combinaisons de type Classic + MLNi,
on constate que pour les modèles de prédiction 
\begin{itemize}
\item Forêt Aléatoire (MLN\_bipart\_intra\_Motif\_max)
\item XGboost (MLN\_Fonction\_et\_Civilité\_Degrée)
\item et Arbre de Décision (MLN\_bipart\_intra\_Fonction\_Sit\_Matrim)
\end{itemize}
les descripteurs issus du graphe multicouches sont mieux classés que les attributs associés sous leur formes classique
...
\newpage
\begin{itemize}
	\item Cas du modèle SVM – meilleur graphe multicouche MLN2
	
	\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/sv_afb_shapley.png}
\end{figure}


	\item Cas du modèle XGBoost – meilleur graphe multicouche MLN1
	
	\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/xgb_afb_shapley.png}
\end{figure}


	\item Cas du modèle Arbre de Décision (Decision Tree) – meilleur graphe multicouche MLN2
	
	\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/dtc_afb_shapley.png}
\end{figure}


	\item Cas du modèle Régression Logistique – meilleur graphe multicouche MLN3
	
	\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/lrc_afb_shapley.png}
\end{figure}


	\item Cas du modèle Forêt Aléatoire (Random Forest) – meilleur graphe multicouche MLN3
	
	\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/rfc_afb_shapley.png}
\end{figure}
\end{itemize}
...
L'équation de calcul itératif du PageRank personnalisé à un seul emprunteur est donnée par:
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/AER_G_vs_P_up}
  \stepcounter{tableau}
   \captionsetup{labelformat=tableau}
  \caption{Pourcentage (\%) de gain PageRank avec personnalisation global versus PageRank avec personnalisation à un seul emprunteur pour le jeu de données AER de Kaggle}
  \label{fig:gpaerup}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/AFB_G_vs_P_up}
  \stepcounter{tableau}
   \captionsetup{labelformat=tableau}
  \caption{Pourcentage (\%) de gain PageRank avec personnalisation global versus PageRank avec personnalisation à un seul emprunteur pour le jeu de données AFB}
  \label{fig:gpafbup}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/CREDITR_G_vs_P_up}
  \stepcounter{tableau}
   \captionsetup{labelformat=tableau}
  \caption{Pourcentage (\%) de gain PageRank avec personnalisation global versus PageRank avec personnalisation à un seul emprunteur pour le jeu de données CREDIT RISK}
  \label{fig:gpcreditrup}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/GERMAN_G_vs_P_up}
   \captionsetup{labelformat=tableau}
  \caption{Pourcentage (\%) de gain PageRank avec personnalisation global versus PageRank avec personnalisation à un seul emprunteur pour le jeu de données GERMAN}
  \label{fig:gpgermanup}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/JAPAN_G_vs_P_up}
  \stepcounter{tableau}
   \captionsetup{labelformat=tableau}
  \caption{Pourcentage (\%) de gain PageRank avec personnalisation global versus PageRank avec personnalisation à un seul emprunteur pour le jeu de données JAPAN}
  \label{fig:gpjapanup}
\end{figure}

Nous remarquons que dans les résultats ci-haut que bien qu'il y ai une sorte d'équivalence entre la personnalisation globale et celle à un seul emprunteur, bien de fois, la version personnalisation à un seul emprunteur l'emporte sur globale et nous battons ainsi le meilleur modèle issu de ces jeux de données. De plus, nous pouvons aussi visualiser les résultats d'analyse d'importance des attributs dans la décision suivant selon les modèles:
\newpage
\begin{itemize}
\item \ac{SVM}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/sv_shapley.png}
\end{figure}
\item \ac{LR}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/lrc_shapley.png}
\end{figure}
\item \ac{DT}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/dtc_shapley.png}
\end{figure}
\item \ac{RF}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/rfc_shapley.png}
\end{figure}
\item \ac{XGB}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{assets/xgb_shapley.png}
\end{figure}
\end{itemize}
...
Lors de ces expérimentation, Nous avons remarqué la non importance de plusieurs attributs des jeux de données dans les modèles, l'absence d'information de classe dans le graphe multicouche biparti construit, la minorité d'attributs extraits des graphes. Pour cela nous pensons au perspectives suivantes:
\begin{enumerate}
\item définir un processus de présélection des attributs avant entrainement
\item intégrer les classes dans la modélisation du graphe multicouche
\item exploitation différente du graphe multicouche sans avoir recourt au PageRank.
\end{enumerate}
