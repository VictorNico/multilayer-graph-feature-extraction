        ...
        'MlC':{
            'GLO':{
                'MX': [
                    load_data_set_from_url(path=file, sep='\t', encoding='utf-8', index_col=0, na_values=None)
                    for file in get_filenames(
                        root_dir=f'{outputs_path}/{alpha}/{type}/{"mlna_" + str(k) if k == 1 else "mlna_" + str(k) + "_b"}/global/data_selection_storage',
                ...
                'CX': [
                    load_data_set_from_url(path=file, sep='\t', encoding='utf-8', index_col=0, na_values=None)
                    for file in get_filenames(
                        root_dir=f'{outputs_path}/withClass/{alpha}/{type}/{"mlna_" + str(k) if k == 1 else "mlna_" + str(k) + "_b"}/global/data_selection_storage',
                        func=lambda x: (MlC_F(x)),
                        verbose=False
                    )
                ]
            ...
            'PER':{
                'MX': [
                    load_data_set_from_url(path=file, sep='\t', encoding='utf-8', index_col=0, na_values=None)
                    for file in get_filenames(
                        root_dir=f'{outputs_path}/{alpha}/{type}/{"mlna_" + str(k) if k == 1 else "mlna_" + str(k) + "_b"}/personalized/data_selection_storage',
                        func=lambda x: (MlC_F(x)),
                        verbose=False
                    )
                ],
                'CX': [
                    load_data_set_from_url(path=file, sep='\t', encoding='utf-8', index_col=0, na_values=None)
                    for file in get_filenames(
                        root_dir=f'{outputs_path}/withClass/{alpha}/{type}/{"mlna_" + str(k) if k == 1 else "mlna_" + str(k) + "_b"}/personalized/data_selection_storage',
                        func=lambda x: (MlC_F(x)),
                        verbose=False
                    )
                ],
            ...
            'GAP':{
                'MX': [
                    load_data_set_from_url(path=file, sep='\t', encoding='utf-8', index_col=0, na_values=None)
                    for file in get_filenames(
                        root_dir=f'{outputs_path}/{alpha}/{type}/{"mlna_" + str(k) if k == 1 else "mlna_" + str(k) + "_b"}/mixed/data_selection_storage',
                        func=lambda x: (MlC_F(x)),
                        verbose=False
                    )
                ],
                'CX': [
                    load_data_set_from_url(path=file, sep='\t', encoding='utf-8', index_col=0, na_values=None)
                    for file in get_filenames(
                        root_dir=f'{outputs_path}/withClass/{alpha}/{type}/{"mlna_" + str(k) if k == 1 else "mlna_" + str(k) + "_b"}/mixed/data_selection_storage',
                        func=lambda x: (MlC_F(x)),
                        verbose=False
                    )
                ],
...
def compare_MlC_MCA_for_GLO_PER_GAP(
    outputs_path=None, 
    cwd=None, 
    data_folder=None, 
