...
def print_compare_v3_2(
    ...
    for folder in list(tables.keys()):
        ...
        max_config = lambda store, mod, folder, metric, app, act: (([round(act(store[mod][folder][metric][app][logic][config]),2) for logic in store[mod][folder][metric][app].keys() for config in store[mod][folder][metric][app][logic].keys()]))
                ...
                for i, metric in enumerate(metrics):
                    ...
                    for li,logic in enumerate(list(store[model][folder][metric][approach].keys())):
                        for ci,config in enumerate(list(store[model][folder][metric][approach][logic].keys())): # MX, CX, CY, CXY 
                            ...
                            act = max if valll is False else (min if "financial-cost" in metric else max)
                            act_b = sorted(list(set(max_config(store, model, folder, metric, approach, act))), reverse= (False if ((valll == True) and ("financial-cost" in metric )) else True))
                            pos = act_b.index(round(act(store[model][folder][metric][approach][logic][config]),2))
                            ...
                            form = f"{int_part}.{str(act(store[model][folder][metric][approach][logic][config])).split('.')[1][:decimal_digits]}" if total_digits != None else str(round(act(store[model][folder][metric][approach][logic][config]),2))
                            ...
                            lines+= f"& {val}"
                            bests = max_config(store, model, folder, metric, approach, act)
                            counterStore[metric][logic][config].append(1/bests.count(round(act(store[model][folder][metric][approach][logic][config]),2)) if pos == 0 else 0)
                            counterStore_1[metric][logic][config].append(1/bests.count(round(act(store[model][folder][metric][approach][logic][config]),2)) if pos == 1 else 0)
                            counterStore_2[metric][logic][config].append(1/bests.count(round(act(store[model][folder][metric][approach][logic][config]),2)) if pos == 2 else 0)
                # ---------------------------------------
...
def generate_report_tables_v3_2(
                        ...
                        for approach in list(ig_local_details_metrics_depth_1[model][result_folder][metric].keys()): # Mlc or MCA
                            ...
                            for logic in list(ig_local_details_metrics_depth_1[model][result_folder][metric][approach].keys()): # GLO, PER or GAP
                                for config in list(ig_local_details_metrics_depth_1[model][result_folder][metric][approach][logic].keys()): # MX, CX, CY, CXY
                                    ...
                                    # print(f"Approach: {approach}, Logic: {logic}, Config: {config}")
                                    # print(f"{files.get(approach)}")
                                    # print(f" {files.get(approach, {}).get(logic)}")
                                    # print(f"{files.get(approach, {}).get(logic, {}).get(config)}")
                                    ...
                                    for result in list(range(len(files[approach][logic][config]))): # each result file's containing evaluation metrics
                                        ...
                                        # print(main_key,met, result_folder)
    ...
    joblib.dump(best_mlna_k_per_alpha, f'{cwd}/{outputPath}/best.tex')
    joblib.dump(macro_metrics, f'{cwd}/{outputPath}/macro_metrics.tex')
