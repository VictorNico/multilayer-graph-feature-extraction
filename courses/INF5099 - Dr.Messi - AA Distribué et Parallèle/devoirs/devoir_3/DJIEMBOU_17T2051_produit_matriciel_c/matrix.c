...
void* modulo_matrix_multiply_thread(void* arg) {
    ...
    long start_row = (thread_id * rows1) / NUM_THREADS;
    double** A = _ThreadArgs.A;
    double** B = _ThreadArgs.B;
    double** E = _ThreadArgs.E;
    int i, j, k;
...
void* block_matrix_multiply_thread(void* arg) {
    ...
    long start_row = (thread_id * rows1) / NUM_THREADS;
    long end_row = ((thread_id + 1) * rows1) / NUM_THREADS;

    double** A = _ThreadArgs.A;
    double** B = _ThreadArgs.B;
    double** D = _ThreadArgs.D;
    int i, j, k;
    // printf("-----.--");
...
void modulo_par_matrix_multiply(double** A, double** B, double** C, int cols1_rows2, int cols2, int rows1, int NUM_THREADS) {
    ...
    for (i = 0; i < NUM_THREADS; i++) {
        pthread_create(&threads[i], NULL, modulo_matrix_multiply_thread, (void*)i);
    }
...
void block_par_matrix_multiply(double** A, double** B, double** C, int cols1_rows2, int cols2, int rows1, int NUM_THREADS) {
    ...
    for (i = 0; i < NUM_THREADS; i++) {
        pthread_create(&threads[i], NULL, block_matrix_multiply_thread, (void*)i);
    }
