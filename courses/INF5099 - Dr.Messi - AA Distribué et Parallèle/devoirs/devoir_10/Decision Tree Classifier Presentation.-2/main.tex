...
\begin{frame}
	\frametitle{\textsc{Stratégies de parallélisation}}
\begin{itemize}
\item Environnement UMA
\item Parallélisation par modulo
\item Approches (Mask, IG, Best Split, Growth)
\item Métri	ques (Accuracy, F1-score, Speedup)
\end{itemize}
...
\begin{frame}[allowframebreaks]
	\frametitle{\textsc{Expérimentation}}
	%\Background
	\begin{table}[H]
	...
	\begin{block}{evaluation du temps}
...
\section{Discussion}
\begin{frame}
	\frametitle{\textsc{Discussion}}
	\begin{block}{Mask, IG, ID Growth}
	Sachant $n$ threads, $m$ variables à $k$ valeurs chacune, nous aurons $ n \times m \times k $ création et jointure de threads pour le traitement d'un seul noeud. Ceci a un coût non négligeable.
	\end{block}
	
	\begin{block}{Best Split}
	Sachant $n$ threads, $m$ variables, nous avons  $ n \times m $ création et jointure de threads pour le traitement d'un seul noeud. Ceci qui est mieux que les autres. et en utilisant juste 2 threads, nous réduisons les coûts et le temps.
	\end{block}
	\begin{alertblock}{Meilleur Configuration}
	Parallelisation de la recherche du meilleur candidat séparateur avec 2 threads pour un speedup de 4.7 et 5.5 pour les jeux de données GERMAN et CREDIT RISK 
	\end{alertblock}
\end{frame}
