...
// Fonction getMask
std::vector<bool> getMask(const std::vector<double>& X, std::function<bool(const double&, const double&)> cond, const double& val) {
    std::vector<bool> mask;
    for (double x : X) {
        mask.push_back(cond(x, val));
    }
    return mask;
}

double information_gain(const std::vector<double>& y,
    ...
    if (a == 0 || b == 0) {
        ...
        std::vector<double> pos_data, neg_data;
...
std::tuple<double, double, bool, bool> max_information_gain_split(
    ...
    if (!x.empty()) {
        std::string data = std::to_string(x[0]);
        numeric_variable = std::all_of(data.begin(), data.end(), ::isdigit);
    }

    std::vector<double> options;
    if (numeric_variable) {
        std::set<double> unique_options(x.begin(), x.end());
    ...
    if (ig.empty()) {
        ...
        double best_split = split_value[best_ig_index];
...
get_best_split(const std::string& y,
    ...
    std::unordered_map<std::string, std::tuple<double, double, bool, bool>> masks;
    ...
    if (!any_valid) {
        return std::make_tuple(std::string(), double(), 0.0, false);
    } else {
        ...
        double split_value = std::get<1>(masks[split_variable]);
...
make_split(const std::string& variable,
    ...
    std::unordered_map<std::string, std::vector<double>> data_1;
    std::unordered_map<std::string, std::vector<double>> data_2;
...
double makePrediction(const std::vector<double>& data, bool targetFactor) {
    std::unordered_map<double, size_t> doubleFrequencyMap;

    for (double value : data) {
        doubleFrequencyMap[value]++;
    }
...
std::pair<bool, bool> checkConditionsDTree(int depth, const std::unordered_map<std::string, std::vector<double>>& xy_current, int max_categories, int max_depth, int counter, int min_samples_split) {
...
void sub_tree(
        ...
        std::unordered_map<std::string, std::vector<double>> left,
        std::unordered_map<std::string, std::vector<double>> right) {
...
void leaf_tree(std::unordered_map<std::string, std::vector<double>> xy_current,
               std::string y, bool target_factor, std::string var, double ig,
               double val,
               TreeNode* current_node) {
//    std::cout << "pre pred" << std::endl;
    double pred = makePrediction(xy_current[y], target_factor);
...
TreeNode* interativeTrainTree(
    ...
    while (!stack.empty()) {
        ...
        stack.pop_back();
        std::unordered_map<std::string, std::vector<double>> xy_current = current.data;
...
//            std::cout << "pre leaf 2" << std::endl;
            leaf_tree(xy_current, y, target_factor, std::string(), std::numeric_limits<double>::infinity(), double(), current_node);
...
double predict(
        const std::unordered_map<std::string, double>& observation,
        TreeNode* tree_node) {

    //  if root is leaf
    if (!std::holds_alternative<std::string>(tree_node->condition)) {
            return std::get<double>(tree_node->condition);
    }
    ...
    TreeNode* answer;
    const double& first_element = observation.at(parts[0]);
    ...
    if (!std::holds_alternative<std::string>(answer->condition)) {
        return std::get<double>(answer->condition);
    }
...
std::vector<double> predictions(
        std::unordered_map<std::string, std::vector<double>> test_data, TreeNode* IDT){
    // prediction vector
