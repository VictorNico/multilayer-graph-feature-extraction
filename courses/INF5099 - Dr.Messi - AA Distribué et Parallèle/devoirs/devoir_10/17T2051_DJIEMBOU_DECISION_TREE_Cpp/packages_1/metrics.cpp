...
std::vector<std::vector<int>> compute_confusion_matrix(
        ...
        const std::vector<double>& predicted_labels,
    ...
    // Create a dictionary to map labels to their index
    std::unordered_map<double, int> label_to_index;
    for (int i = 0; i < num_classes; i++) {
        label_to_index[labels[i]] = i;
    }
...
std::unordered_map<std::string, int> count_elements(const std::vector<double>& lst) {
    /**
