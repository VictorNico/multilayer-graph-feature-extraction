...
std::unordered_map<std::string, std::vector<double>> process_file(const std::string& file_path, const std::string& details_separator, bool header_flag) {
    /**
     ...
     */
    std::unordered_map<std::string, std::vector<double>> descriptions;
            ...
            while (std::getline(iss, field, *details_separator.c_str())) {
                ...
                if (header.empty()) {
                    if (descriptions.find(std::to_string(fieldIndex)) == descriptions.end()) {
                        descriptions[std::to_string(fieldIndex)] = std::vector<double>();
                    }
                    ...
                    if (descriptions.find(header[fieldIndex]) == descriptions.end()) {
                        descriptions[header[fieldIndex]] = std::vector<double>();
                    }
    ...
    if (descriptions.find("Index") == descriptions.end()) {
        ...
        descriptions["Index"] = std::vector<double>();
...
double castToAppropriateType(const std::string& data) {
    ...
    try {
        return std::stod(data);
         ...
         throw std::invalid_argument("Les données doivent être de type numériques");
    }
}
...
std::tuple<
    ...
    std::vector<double> train_labels(indexes.size() - test_count);
    std::vector<double> test_labels(test_count);
    ...
    for (const auto& key : data) {
        std::vector<double> train_values(indexes.size() - test_count);
        std::vector<double> test_values(test_count);
        for (size_t i = 0; i < indexes.size(); ++i) {
            if (i < indexes.size() - test_count) {
