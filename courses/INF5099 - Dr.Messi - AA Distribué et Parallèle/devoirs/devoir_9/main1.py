if __name__ == '__main__':
    ...
    del dat['Index']
    del test_data['Index']
    print(f"Training data size: {len(dat)} {len(dat[list(test_data.keys())[0]])} {dat.keys()}")
    print(f"Test data size: {len(test_data)} {len(test_data[list(test_data.keys())[0]])}")
    print(f"""
        {count_elements(dat['Class'])}
        ------------------------
        {count_elements(test_labels)}

        """)

    IDT = interative_train_tree(dat, 'Class', True)
    result1 = []
    ...
    write_file("data/pl.txt", f"{dat}")
    write_file("data/tl.txt", f"{test_data}")
    write_file("data/l.txt", f"{test_labels}")
