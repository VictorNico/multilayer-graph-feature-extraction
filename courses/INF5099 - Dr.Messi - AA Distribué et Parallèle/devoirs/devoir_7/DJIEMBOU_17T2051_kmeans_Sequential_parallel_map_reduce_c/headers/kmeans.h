...
typedef struct {
    ...
    int* assignments_mr; /** vector of assignments by map reduce */
    ...
    Point* centroids_mr; /** vector of centroids by map reduce */
 ...
 * \brief Storage structure
 * 
 * This structure store all point belong to a specific luster
 */
typedef struct {
    int* points; /** vector of data point */
    int pointSize; /** size of points in cluster */    
} Cluster;
