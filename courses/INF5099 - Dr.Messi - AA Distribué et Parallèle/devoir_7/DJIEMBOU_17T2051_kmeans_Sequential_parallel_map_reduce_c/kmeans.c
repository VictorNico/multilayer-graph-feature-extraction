...
void* map(void* arg) {
    ...
    for (int i = 0; i < _ThreadArgs.k; i++){
        ...
        localCluster[i].points = malloc(initialSize*sizeof(Point));
    }

    // Attribution des points aux clusters
    for (int i = start; i < end; i++) {
        // recuperer l'indice du cluster d'assignation
        indice = assignCluster(_ThreadArgs.points[i], _ThreadArgs.centroids_mr, _ThreadArgs.k, _ThreadArgs.dim);
        _ThreadArgs.assignments_mr[i] = indice;

        // sauvegarder le dernier indice occupé
        prev = localCluster[indice].pointSize;
        // incrementer le nombre de points dans le cluster
        localCluster[indice].pointSize += 1;
        // reallouer le vecteur de 
        if(localCluster[indice].pointSize > initialSize){
            localCluster[indice].points = realloc(localCluster[indice].points,localCluster[indice].pointSize*sizeof(Point));
        }
        // ajouter le nouveau point
        localCluster[indice].points[prev] = _ThreadArgs.points[i];
    }
...
void* reduce(void* arg){
    ...
    if(thread_id == (_ThreadArgs.NUM_THREADS - 1))
       ...
       end =  begin + (_ThreadArgs.k/_ThreadArgs.NUM_THREADS);

    Point* sum = malloc(_ThreadArgs.k * sizeof(Point));
    for (int i = 0; i < _ThreadArgs.k; i++) {
        sum[i].dim = malloc(_ThreadArgs.dim * sizeof(double));  // Allocate memory for sum[i].dim
    }
    ...
    for (int i = begin; i < end; i++) {
        if (_clusters[i].pointSize > 0) {
            ...
            // pthread_mutex_lock (&mutex_variable);
            memset(sum[i].dim, 0, _ThreadArgs.dim * sizeof(double)); // Initialize sum[i].dim
            memset(_ThreadArgs.centroids_mr[i].dim, 0, _ThreadArgs.dim * sizeof(double));
            // pthread_mutex_unlock (&mutex_variable);
            // somme des points du cluster
            for (int j = 0; j < _clusters[i].pointSize; j++) {
                for (int k = 0; k < _ThreadArgs.dim; k++) {
                    sum[i].dim[k] += _clusters[i].points[j].dim[k];
                }
            }

            // calculer le nouveau centroide
            for (int j = 0; j < _ThreadArgs.dim; j++) {
                _ThreadArgs.centroids_mr[i].dim[j] = sum[i].dim[j] / _clusters[i].pointSize;
            }
        }
